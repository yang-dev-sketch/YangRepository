...
import { StyleSheet, Text, View, ScrollView, Platform, ViewPropTypes } from 'react-native';
...
export default class ScrollPicker extends React.Component {
  constructor(props) {
    ...
    this.wrapperHeight = fixedHeight
      ? wrapperHeight
      : dataSource.length * itemHeight < wrapperHeight
  ...
  handleWrapperHeightChange = () => {
    const { dataSource, itemHeight, wrapperHeight, fixedHeight } = this.props;
  ...
  render() {
    ...
    const { wrapperStyle, highlightStyle, itemHeight, rotationEnabled } = this.props;
    ...
    return (
      <View style={[{ height: this.wrapperHeight }, wrapperStyle]}>
        <View
          style={[
            {
